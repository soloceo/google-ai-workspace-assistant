        ...
        <div className="flex-1 flex overflow-hidden">
          {activeTab === "ai" ? (
            <AIChatPanel
              ...
              onCreateEvent={async (accountEmail, event) => {
                const token = authService.getValidToken(accountEmail);
                if (!token) { toast.error(t.noValidToken); return; }
                await calendarService.createEvent(token, event);
                toast.success(lang === "zh" ? "日程已创建" : "Event created");
                loadData();
              }}
            />
          ) : (
...
function CalendarQuickCreate({ lang, t, accounts, onCreateEvent }: {
  lang: string;
  t: any;
  accounts?: { email: string; name?: string }[];
  onCreateEvent: (accountEmail: string, event: { summary: string; start: string; end: string; description?: string }) => Promise<void>;
}) {
  ...
  const handleCreate = async () => {
    if (!summary.trim() || !start || !end || !selectedAccount) return;
    setCreating(true);
    try {
      await onCreateEvent(selectedAccount, {
  ...
  if (created) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 text-[12px]">
        <CheckCircle2 className="h-3.5 w-3.5" />
        {lang === "zh" ? "日程已创建" : "Event created"}
      </div>
    );
  }
  ...
  if (!expanded) {
    return (
      <button
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gm-border bg-gm-bg text-[12px] font-medium text-gm-text hover:bg-gm-blue-bg hover:border-gm-blue hover:text-gm-blue transition-colors w-fit"
      >
        <CalendarIcon className="h-3.5 w-3.5" />
        {lang === "zh" ? "创建日程" : "Create Event"}
      </button>
    );
  }

  return (
    <div className="rounded-xl border border-gm-border bg-gm-bg p-3 space-y-2.5 max-w-sm">
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-semibold text-gm-text-secondary uppercase tracking-wider">
          {lang === "zh" ? "快速创建日程" : "Quick Create Event"}
        </span>
        <button onClick={() => setExpanded(false)} className="text-gm-text-secondary hover:text-gm-text p-0.5">
      ...
      {accounts && accounts.length > 1 && (
        <select
          ...
          className="w-full text-xs bg-gm-bg-container border border-gm-border rounded-lg px-2.5 py-2 text-gm-text outline-none focus:ring-1 focus:ring-gm-blue"
        >
          {accounts.map(a => (
      ...
      <input
        ...
        value={summary}
        onChange={(e) => setSummary(e.target.value)}
        className="w-full text-sm bg-gm-bg-container border border-gm-border rounded-lg px-2.5 py-2 text-gm-text outline-none focus:ring-1 focus:ring-gm-blue"
      />
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-[10px] text-gm-text-secondary">{lang === "zh" ? "开始" : "Start"}</label>
          <input
            type="datetime-local"
            value={start}
            onChange={(e) => setStart(e.target.value)}
            className="w-full text-xs bg-gm-bg-container border border-gm-border rounded-lg px-2 py-1.5 text-gm-text outline-none focus:ring-1 focus:ring-gm-blue"
          />
        </div>
        <div>
          <label className="text-[10px] text-gm-text-secondary">{lang === "zh" ? "结束" : "End"}</label>
          <input
            type="datetime-local"
            value={end}
            onChange={(e) => setEnd(e.target.value)}
      ...
      <textarea
        placeholder={lang === "zh" ? "备注（可选）" : "Description (optional)"}
        value={description}
        onChange={(e) => setDescription(e.target.value)}
        className="w-full text-xs bg-gm-bg-container border border-gm-border rounded-lg px-2.5 py-2 text-gm-text outline-none focus:ring-1 focus:ring-gm-blue resize-none h-16"
      />
      <button
        onClick={handleCreate}
        disabled={!summary.trim() || !start || !end || creating}
        className="w-full py-2 rounded-lg bg-[#1a73e8] hover:bg-[#1557b0] text-white text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 transition-colors"
      >
        {creating ? <RefreshCw className="h-3 w-3 animate-spin" /> : <CalendarIcon className="h-3 w-3" />}
        {creating ? (lang === "zh" ? "创建中..." : "Creating...") : (lang === "zh" ? "创建日程" : "Create Event")}
      </button>
    </div>
  );
}

function AIChatPanel({ messages, input, setInput, streaming, onSend, chatEndRef, lang, t, isMobile, onClearChat, data, onDraftReply, onArchiveEmail, onSelectEmail, onCreateEvent, accounts }: {
            ...
            <div key={msg.id}>
              <div className={cn("flex gap-3 py-3", msg.role === "user" ? "justify-end" : "justify-start")}>
                ...
                <div className="max-w-[85%] group/msg relative">
                  ...
                  {/* Quick actions: create event + follow-up chips */}
                  {msg.role === "assistant" && msg.text && !streaming && (() => {
                    const txt = msg.text.toLowerCase();
                    const isLastMsg = msgIdx === messages.length - 1;
                    const hasCalendarIntent = /创建.*日[程历]|安排.*会议|schedule|create.*event|book.*meeting|set up.*meeting|添加.*日[程历]/.test(txt);

                    return (
                      <div className="flex flex-col gap-2 mt-2">
                        {hasCalendarIntent && onCreateEvent && (
                          <CalendarQuickCreate
                            lang={lang}
                            t={t}
                            accounts={accounts}
