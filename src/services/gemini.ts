...
export const WORKSPACE_TOOLS = [{
  functionDeclarations: [
    ...
    {
      name: "search_notes",
      description: "Search the user's personal notebook. Notes can contain text and photos of products, receipts, labels, ideas — photo text is OCR'd, so brand names, prices, model numbers from pictures are searchable. Notes in the 'accounting' category also carry structured amount/tax/payment data and represent income or expense entries. Use this for \"how much did I spend on X\", \"what did I photograph of Y\", \"what did I note about Z\" questions.",
      parameters: {
        type: Type.OBJECT,
        properties: {
          query: { type: Type.STRING, description: "Keywords to match against note titles, body text, and OCR'd photo text. Multiple words are ANDed together. Pass empty string to list every note (useful with category filter)." },
          category: { type: Type.STRING, description: "Optional filter: 'product', 'idea', 'task', 'accounting' (income/expense entries), or 'other'." },
        },
        required: ["query"],
      },
    },
...
- When the user asks about something they **noted down, photographed, saved to their notebook, or recorded as income/expense** (products, wine labels, receipts, ideas, items to remember, commissions, ad spend, reimbursements), use the **search_notes** tool. Photo text is OCR'd, so brand names, prices, model numbers from pictures are searchable. The **accounting category** holds structured financial entries with amount/tax/payment — use it for questions like "我这个月花了多少广告费", "本月佣金收入", "how much did I spend on staging", "what receipts did I log last week". When a user asks for a total, call search_notes with category=accounting and the executor will attach a pre-computed totals line (income, expense, net) to the result.
