...
// KV key: tx:owner:${email}:${txId}
// Each transaction carries its own tax mode and rate so the user can
// mix GST-only, HST-included, and tax-exempt entries freely.

const TX_TYPES = new Set(['income', 'expense']);
const TX_TAX_MODES = new Set(['exclusive', 'inclusive', 'exempt']);
const TX_PAYMENTS = new Set(['cash', 'credit', 'bank', 'cheque', 'other']);
const MAX_TX_SIZE_BYTES = 64 * 1024;
...
interface StoredTransaction {
  ...
  category: string;           // free text; UI offers presets
  description?: string;
  dealId?: string;            // optional link back to a Deal
  owner?: string;
}
...
function sanitizeTx(n: any, existing?: StoredTransaction): StoredTransaction {
  const now = Date.now();
  const num = (v: any, fallback: number): number => {
    const f = Number(v);
    return Number.isFinite(f) ? f : fallback;
  };
  const str = (v: any, fallback: string, max: number): string => {
    if (typeof v === 'string' && v) return v.slice(0, max);
    return fallback;
  };
  const date = typeof n?.date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(n.date)
    ? n.date
    : existing?.date || new Date().toISOString().slice(0, 10);
  return {
    id: existing?.id || newTxId(),
    created_at: existing?.created_at || now,
    updated_at: now,
    date,
    type: TX_TYPES.has(n?.type) ? n.type : existing?.type || 'expense',
    amount: num(n?.amount ?? existing?.amount, 0),
    taxMode: TX_TAX_MODES.has(n?.taxMode) ? n.taxMode : existing?.taxMode || 'exclusive',
    taxRate: num(n?.taxRate ?? existing?.taxRate, 13),
    payment: TX_PAYMENTS.has(n?.payment) ? n.payment : existing?.payment || 'credit',
    category: str(n?.category, existing?.category || 'other', 100),
    description: typeof n?.description === 'string' ? n.description.slice(0, 1000) : existing?.description,
    dealId: typeof n?.dealId === 'string' ? n.dealId.slice(0, 64) : existing?.dealId,
  };
}

async function findTxKey(env: Env, sid: string, txId: string): Promise<{ key: string; owner: string } | null> {
  const emails = await getSessionEmails(env, sid);
  if (emails.length === 0) return null;
  const primary = await getPrimaryOwnerForSession(env, sid);
  const order = primary && emails.includes(primary)
    ...
    : emails;
  for (const email of order) {
    const key = `tx:owner:${email}:${txId}`;
    if (await env.TOKENS.get(key)) return { key, owner: email };
  }
  return null;
}

async function handleListTx(req: Request, env: Env): Promise<Response> {
  const sid = getSessionId(req);
  if (!sid) return json(env, { transactions: [] });
  const emails = await getSessionEmails(env, sid);
  if (emails.length === 0) return json(env, { transactions: [] });

  const all: StoredTransaction[] = [];
  for (const email of emails) {
    const { keys } = await env.TOKENS.list({ prefix: `tx:owner:${email}:` });
    const batch = await Promise.all(
      keys.map(async (k) => {
        const raw = await env.TOKENS.get(k.name);
        if (!raw) return null;
        try {
          const t = JSON.parse(raw) as StoredTransaction;
          t.owner = email;
          return t;
        } catch { return null; }
      })
    );
    all.push(...batch.filter((t): t is StoredTransaction => t !== null));
  }
  // Most recent date first; break ties with updated_at
  all.sort((a, b) => (b.date.localeCompare(a.date)) || (b.updated_at - a.updated_at));
  return json(env, { transactions: all });
}

async function handleCreateTx(req: Request, env: Env): Promise<Response> {
  const sid = getSessionId(req);
  if (!sid) return json(env, { error: 'No session' }, { status: 401 });
  const owner = await getPrimaryOwnerForSession(env, sid);
  if (!owner) return json(env, { error: 'Must log in with a Google account first' }, { status: 401 });
  ...
  if (new TextEncoder().encode(serialized).length > MAX_TX_SIZE_BYTES) {
    return json(env, { error: 'Transaction too large' }, { status: 413 });
  }
  await env.TOKENS.put(`tx:owner:${owner}:${tx.id}`, serialized);
  return json(env, { transaction: { ...tx, owner } });
}

async function handleUpdateTx(req: Request, env: Env, txId: string): Promise<Response> {
  const sid = getSessionId(req);
  if (!sid) return json(env, { error: 'No session' }, { status: 401 });
  const found = await findTxKey(env, sid, txId);
  if (!found) return json(env, { error: 'Not found' }, { status: 404 });
  const raw = await env.TOKENS.get(found.key);
  if (!raw) return json(env, { error: 'Not found' }, { status: 404 });
  const existing = JSON.parse(raw) as StoredTransaction;

  const body = (await req.json().catch(() => ({}))) as Record<string, any>;
  const tx = sanitizeTx({ ...existing, ...body }, existing);
  const serialized = JSON.stringify(tx);
  if (new TextEncoder().encode(serialized).length > MAX_TX_SIZE_BYTES) {
    return json(env, { error: 'Transaction too large' }, { status: 413 });
  }
  await env.TOKENS.put(found.key, serialized);
  return json(env, { transaction: { ...tx, owner: found.owner } });
}

async function handleDeleteTx(req: Request, env: Env, txId: string): Promise<Response> {
  const sid = getSessionId(req);
  if (!sid) return json(env, { ok: true });
  const found = await findTxKey(env, sid, txId);
  if (found) await env.TOKENS.delete(found.key);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      // Transactions (income / expense ledger)
      if (url.pathname === '/transactions' && request.method === 'GET') return handleListTx(request, env);
      if (url.pathname === '/transactions' && request.method === 'POST') return handleCreateTx(request, env);
      const txMatch = url.pathname.match(/^\/transactions\/([A-Za-z0-9_-]+)$/);
      if (txMatch && request.method === 'PATCH') return handleUpdateTx(request, env, txMatch[1]);
      if (txMatch && request.method === 'DELETE') return handleDeleteTx(request, env, txMatch[1]);
